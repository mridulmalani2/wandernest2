          ...
          <tr>
            <td style="background-color: #f9fafb; padding: 24px 40px; border-top: 1px solid ${theme.colors.border}; text-align: center;">
              ...
              <div style="margin-bottom: 16px;">
                <a href="${baseUrl}" style="color: ${theme.colors.textSecondary}; text-decoration: none; font-size: 12px; margin: 0 8px; hover: text-decoration: underline;">Home</a>
                <span style="color: ${theme.colors.border};">•</span>
                <a href="${baseUrl}/support" style="color: ${theme.colors.textSecondary}; text-decoration: none; font-size: 12px; margin: 0 8px; hover: text-decoration: underline;">Support</a>
              </div>
...
export const getOtpEmailHtml = (otp: string, title: string = 'Sign In Verification', subtitle: string = 'Use the code below to securely sign in to your account. This code expires in 10 minutes.') => {
  ...
  `;
  return PremiumEmailLayout(content, title);
};
  ...
  const content = `
    <div style="text-align: center;">
      ...
      <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.6; color: ${theme.colors.textSecondary};">
        Your trip to <strong style="color: ${theme.colors.text};">${city}</strong> is officially in the works.
      </p>
      ...
      ${hasMatches ? `
        <div style="background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 12px; padding: 16px; text-align: left; margin-bottom: 24px;">
          ...
          <p style="margin: 0; font-size: 14px; line-height: 1.5; color: #1e3a8a;">
            We found <strong>${matchesCount} student guides</strong> who match your preferences.
          </p>
        </div>
      ` : ''}

      <a href="${baseUrl}/booking/status/${requestId}" style="display: inline-block; background-color: ${theme.colors.primary}; color: white; font-weight: 600; font-size: 15px; padding: 12px 24px; border-radius: 8px; text-decoration: none;">
        View Booking Status
      </a>
    </div>
  `;
...
export const getStudentRequestNotificationHtml = (
  ...
  const content = `
    <div style="text-align: center;">
      ...
      <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.6; color: ${theme.colors.textSecondary};">
        Hi <strong>${studentName}</strong>, a tourist has requested you as their guide.
      </p>

      <a href="${acceptUrl}" style="display: inline-block; background-color: ${theme.colors.primary}; color: white; font-weight: 600; font-size: 15px; padding: 12px 24px; border-radius: 8px; text-decoration: none;">
        View & Accept Request
      </a>
    </div>
  `;
...
export const getStudentMatchInvitationHtml = (
  ...
  const content = `
    <div style="text-align: center;">
      ...
      <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.6; color: ${theme.colors.textSecondary};">
        Hi <strong>${studentName}</strong>, we found a new trip in <strong>${city}</strong> that matches your profile.
      </p>
...
export const getStudentConfirmationHtml = (
  ...
  const content = `
    <div style="text-align: center;">
      ...
      <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.6; color: ${theme.colors.textSecondary};">
        Hi <strong>${studentName}</strong>, you are confirmed for <strong>${city}</strong>.
      </p>
      ...
      <div style="text-align: left; background-color: #f9fafb; border: 1px solid ${theme.colors.border}; border-radius: 12px; padding: 20px; margin-bottom: 24px;">
        ...
        <p style="margin: 0 0 8px; font-size: 14px; color: ${theme.colors.text};">
          <strong style="color: ${theme.colors.textSecondary};">Email:</strong> <a href="mailto:${touristEmail}" style="color: ${theme.colors.primary}; text-decoration: none;">${touristEmail}</a>
        </p>
        ${touristWhatsapp ? `
        <p style="margin: 0 0 8px; font-size: 14px; color: ${theme.colors.text};">
          <strong style="color: ${theme.colors.textSecondary};">WhatsApp:</strong> ${touristWhatsapp}
        </p>
        ` : ''}
        ${touristPhone ? `
        <p style="margin: 0; font-size: 14px; color: ${theme.colors.text};">
          <strong style="color: ${theme.colors.textSecondary};">Phone:</strong> ${touristPhone}
        </p>
        ` : ''}
      </div>
    </div>
...
export const getTouristAcceptanceNotificationHtml = (
  ...
  const content = `
    <div style="text-align: center;">
      ...
      <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.6; color: ${theme.colors.textSecondary};">
        <strong>${studentName}</strong> has accepted your request for <strong>${city}</strong>.
      </p>
      ...
      <div style="background-color: #f9fafb; border-radius: 12px; padding: 24px; border: 1px solid ${theme.colors.border};">
        <div style="margin-bottom: 16px;">
          ...
          <p style="margin: 0; font-size: 15px; color: ${theme.colors.text}; font-weight: 500;">
            ${name} <span style="color: ${theme.colors.textSecondary}; font-weight: 400;">&lt;${email}&gt;</span>
          </p>
        </div>
        ...
        <div style="margin-bottom: 16px;">
          ...
          <p style="margin: 0; font-size: 15px; color: ${theme.colors.text}; font-weight: 500;">${phone}</p>
        </div>
        ` : ''}
